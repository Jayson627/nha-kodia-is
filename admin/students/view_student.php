<?php
$id = $roll = $firstname = $middlename = $lastname = $gender = $dob = $contact = $present_address = $permanent_address = $voter_registration = '';
...
if(isset($_GET['id'])){
    ...
    if($result->num_rows > 0){
        ...
        $fullname = $firstname . ' ' . ($middlename ? $middlename . ' ' : '') . $lastname; // Concatenate first, middle (if exists), last names
        $status = $row['status']; // Assuming 'status' is fetched from the database
    }
    $stmt->close();
}
?>
...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <div class="pl-4"><?= isset($contact_number) ? $contact_number : 'N/A' ?></div>
        </div>
    </div>
</div>
<div class="row">
...
<!-- Voter's Registration Field -->
<div class="row">
    <div class="col-md-12">
        <div class="form-group">
        <label class="control-label text-muted">Voter's Registration</label>
            <div class="pl-4"><?= isset($voter_registration) && $voter_registration == 'yes' ? 'Registered as Voter' : 'Not Registered' ?></div>
        </div>
    </div>
</div>
                    ...
                    <table class="table table-stripped table-bordered" id="academic-history">
                        ...
                        <thead>
                            <tr class="bg-gradient-dark">
                                ...
                                <th class="py-1 text-center">Name of Children</th>
